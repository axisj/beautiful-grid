...
> 상태: 비필수 장기 후보 · 현재 개발 및 테스트 완료 기준에서 제외
> 결정일: 2026-09-15
> 참고: BeautifulGrid `v1.0.13`은 Excel(`.xlsx`) 및 CSV 내보내기를 지원하지만, 셀 수식 입력·계산 엔진은 제공하지 않는다.

## 1. 개요 및 목적
본 문서는 셀 수식 입력·계산에 대한 사용자 수요와 제품 우선순위가 확인될 경우 재검토할 수 있도록 기존 설계 아이디어를 보존합니다. 현재 로드맵의 필수 기능, 릴리스 목표 또는 테스트 완료 조건을 정의하지 않습니다.

Excel 파일 내보내기와 셀 수식 엔진은 별도 기능입니다. 내보내기는 현재 그리드 데이터를 `.xlsx` 파일로 직렬화하고, 셀 수식 엔진은 사용자가 `=SUM(A1:A10)` 같은 값을 입력했을 때 참조 관계를 계산하고 갱신하는 기능입니다.
