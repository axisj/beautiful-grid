...
test.describe('Cell Merge E2E', () => {
  test('renders merged cells with correct rowspan', async ({ page }) => {
    ...
    // The first column 'mainCategory' should merge the first 5 rows ("가전/디지털")
    ...
    await expect(mergedCell).toContainText('가전/디지털');
    ...
    // The second column 'subCategory' should merge the first 3 rows ("컴퓨터 주변기기")
    const subCategoryMergedCell = page.locator('[role="rfdg-body-frozen"] tr[data-ri="0"] td[data-column-index="1"]');
    await expect(subCategoryMergedCell).toContainText('컴퓨터 주변기기');
