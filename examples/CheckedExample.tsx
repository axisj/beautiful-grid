...
const rawData = [
  ...
  [t('아랍에미리트(15+ LFS)', 'United Arab Emirates (15+ LFS)'), 9432, 7565, 5693, 1871, 80, 92, 57],
    ...
    <>
      <div className='mb-3 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm'>
        <div className='flex items-center gap-2'>
          <span className='font-medium text-slate-600'>{t('행 선택 방식', 'Row Selection Mode')}</span>
          <Segmented
            value={selectionMode}
            options={[
              { label: t('Checkbox (복수 선택)', 'Checkbox (Multiple)'), value: 'checkbox' },
              { label: t('Radio (단일 선택)', 'Radio (Single)'), value: 'radio' },
            ]}
            onChange={value => {
        ...
        >
          <span className='mr-1 font-medium text-slate-600'>{t('선택한 키', 'Selected Keys')} ({checkedKeys.length})</span>
          {checkedKeys.length > 0 ? (
            checkedKeys.map(key => <Tag key={String(key)}>{String(key)}</Tag>)
          ) : (
            <span className='text-slate-400'>{t('없음', 'None')}</span>
          )}
        </output>
      </div>
