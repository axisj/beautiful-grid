...
const topics = [
  ...
  t('TypeScript 5.0 마스터 가이드', 'TypeScript 5.0 Master Guide'),
  t('Next.js 14 App Router 실전', 'Next.js 14 App Router in Practice'),
  t('Zustand와 Recoil 상태관리 비교', 'Comparison of Zustand and Recoil State Management'),
  t('Node.js 백엔드 아키텍처 패턴', 'Node.js Backend Architecture Patterns'),
  ...
  t('웹 접근성(A11y) 가이드라인 준수하기', 'Complying with Web Accessibility (A11y) Guidelines'),
  ...
  t('OAuth 2.0 및 JWT 인증 아키텍처', 'OAuth 2.0 and JWT Authentication Architecture'),
  t('Microservices Event-driven 아키텍처', 'Microservices Event-driven Architecture'),
...
const mockData = Array.from({ length: 60 }, (_, index) => {
  ...
  return {
    values: {
      id,
      title: index >= topics.length ? `${topic} (${t('심화 #', 'Advanced #')}${Math.floor(index / topics.length) + 1})` : topic,
      category,
      author,
      views,
      price,
          ...
          extraItems: [
            {
              ...
              onClick: ({ column }) => {
                ...
                alert(t('컬럼명이 복사되었습니다.', 'Column name copied.'));
              },
            },
          ],
        },
    ...
    <>
      <div className='flex flex-wrap items-center justify-between gap-2 p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm'>
        <div className='flex items-center gap-3 text-slate-600'>
          <span>
            <strong>{t('정렬:', 'Sort:')}</strong> {query.sortParams.length}{t('개', 'ea')}
          ...
          <span>|</span>
          <span>
            <strong>{t('필터:', 'Filter:')}</strong> {query.filterParams.length}{t('개', 'ea')}
        ...
        <div className='flex items-center gap-3'>
          <div className='flex items-center gap-2'>
            <span className='text-xs text-slate-500 font-medium'>{t('아이콘 스타일:', 'Icon Style:')}</span>
            <Segmented
              value={iconTheme}
              onChange={val => setIconTheme(val as 'lucide' | 'default')}
              options={[
                { label: t('Lucide 벡터 아이콘 (커스텀)', 'Lucide Vector Icon (Custom)'), value: 'lucide' },
                { label: t('기본 불릿/기호 (Fallback)', 'Default Bullet/Symbol (Fallback)'), value: 'default' },
              ]}
            />
          </div>

          <Button onClick={handleResetQuery}>{t('정렬 / 필터 전체 초기화', 'Reset All Sort / Filter')}</Button>
        </div>
      </div>
