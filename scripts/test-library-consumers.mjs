...
function main() {
  ...
  try {
    ...
    writeProjectFile(
      typesDir,
      'src/index.ts',
      "import type { BGridColumn, BGridColumnGroupNode, BGridContextMenuItem, BGridCsvExportOptions, BGridDataItem, BGridEditorPluginProps, BGridExportData, BGridProps, BGridRef, BGridScrollToRowOptions, BGridSearchOptions } from 'beautiful-grid';\nimport { createSelectEditorPlugin } from 'beautiful-grid/editors';\nimport { createRef } from 'react';\n\nconst gridRef = createRef<BGridRef>();\nconst scrollOptions: BGridScrollToRowOptions = { align: 'end' };\ngridRef.current?.scrollToRow(0, scrollOptions);\n// @ts-expect-error alignment must be a supported row alignment\ngridRef.current?.scrollToRow(0, { align: 'bottom' });\n\ninterface Row {\n  id: string;\n  name: string;\n  status: 'ready' | 'done';\n  meta: { score: number };\n}\n\nconst typedRef = createRef<BGridRef<Row>>();\nconst exportData: BGridExportData<Row> | undefined = typedRef.current?.getExportData({ rows: 'displayed', columns: 'visible' });\nconst csvOptions: BGridCsvExportOptions = { fileName: 'orders.csv', rows: 'checked' };\ntypedRef.current?.exportCsv(csvOptions);\n\nconst statusEditor = createSelectEditorPlugin<Row, Row['status']>({\n  id: 'status',\n  options: [{ value: 'ready', label: 'Ready' }, { value: 'done', label: 'Done' }],\n});\n\nconst columns: BGridColumn<Row>[] = [\n  { id: 'id', key: 'id', label: 'ID', width: 120, exportable: true, exportHeader: 'Identifier' },\n  { id: 'name', key: 'name', label: 'Name', width: 160, editable: true, editor: { type: 'text' }, getSearchText: ({ value }) => String(value), getExportValue: ({ value }) => String(value).toUpperCase() },\n  {\n    id: 'status',\n    key: 'status',\n    label: 'Status',\n    width: 120,\n    editable: true,\n    editTrigger: 'click',\n    editor: statusEditor,\n    editorIcon: { render: '⌄', ariaLabel: 'Select status' },\n    onChangeValue: ({ changes, commit }) => commit(changes),\n  },\n  { id: 'score', key: ['meta', 'score'], label: 'Score', width: 120, searchable: false },\n];\n\nfunction commitFromPlugin(props: BGridEditorPluginProps<Row>) {\n  void props.commit([{ key: 'status', value: 'done' }]);\n  // @ts-expect-error plugin commit requires a change array, not a single cell value\n  void props.commit('done');\n}\n\nconst columnGroups: BGridColumnGroupNode[] = [{\n  id: 'identity',\n  label: 'Identity',\n  children: ['id', { id: 'details', label: 'Details', children: ['name', 'status', 'score'] }],\n}];\n\nconst data: BGridDataItem<Row>[] = [{ values: { id: '1', name: 'Tom', status: 'ready', meta: { score: 10 } } }];\nconst searchOptions: BGridSearchOptions<Row> = { defaultQuery: 'Tom', onQueryChange: query => void query };\nconst contextItems: BGridContextMenuItem<Row>[] = [{ id: 'inspect', label: 'Inspect', onSelect: target => void target.sourceIndex }];\n\nconst props: BGridProps<Row> = {\n  ref: gridRef,\n  width: 640,\n  height: 320,\n  columns,\n  columnGroups,\n  data,\n  rowKey: 'id',\n  searchOptions,\n  contextMenuOptions: { items: () => contextItems },\n};\n\nvoid props;\nvoid commitFromPlugin;\nvoid exportData;\n",
    );
    writeProjectFile(
      typesDir,
      'src/tree.ts',
      "import type { BGridProps, BGridTreeOptions } from 'beautiful-grid';\n\ninterface TreeRow { id: string; parentId: string | null; name: string }\n\nconst tree: BGridTreeOptions<TreeRow> = {\n  parentRowKey: 'parentId',\n  treeColumnId: 'name',\n  expandedRowKeys: ['root'],\n  onExpandedRowKeysChange: (_keys, event) => {\n    event.item.values.name.toUpperCase();\n    event.sourceIndex.toFixed();\n  },\n  icons: { expanded: '-', collapsed: '+', leaf: '·' },\n};\n\nconst props: BGridProps<TreeRow> = { width: 640, height: 320, columns: [], rowKey: 'id', tree };\n// @ts-expect-error parentRowKey is required\nconst invalidTree: BGridTreeOptions<TreeRow> = { treeColumnId: 'name' };\nvoid props;\nvoid invalidTree;\n",
    );
