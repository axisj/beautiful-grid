...
> [!WARNING]
> **모바일 화면에서의 틀고정 (`frozenColumnIndex`) 반응형 처리**:
> 모바일 기기처럼 화면 폭이 좁은 환경에서는 고정 컬럼 수가 많을 경우 고정 영역이 화면의 대부분을 가려 **가로 스크롤이 작동하지 않는 것처럼 오해**할 수 있습니다.
> 모바일 환경에서는 `containerWidth < 640 ? 1 : 3`처럼 화면 폭에 맞춰 `frozenColumnIndex`를 1개로 줄이거나 고정을 해제(`0`)하는 반응형 구성을 권장합니다.
