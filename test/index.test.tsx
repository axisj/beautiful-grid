...
describe('BGrid wheel scroll behavior', () => {
  interface Row {
    id: number;
    name: string;
  }
  ...
  const data = Array.from({ length: 50 }, (_, idx) => ({ values: { id: idx + 1, name: `row-${idx + 1}` } }));
  ...
  it('consumes wheel when inner scroll can continue', async () => {
    const { container } = render(<BGrid<Row> width={400} height={180} columns={columns} data={data} />);

    const scrollContainer = container.querySelector("[role='rfdg-scroll-container']") as HTMLDivElement;
    expect(scrollContainer).toBeTruthy();

    setScrollableMetrics(scrollContainer, {
      scrollTop: 120,
      clientHeight: 100,
      scrollHeight: 1200,
    });
  ...
  it('does not consume wheel at bottom boundary so page can scroll', async () => {
    const { container } = render(<BGrid<Row> width={400} height={180} columns={columns} data={data} />);
